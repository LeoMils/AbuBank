...
const MUTANTS = [
  ...
  // ── Layer A · code added since the last mutation run (resolvers, detectors, online loop) ──
  {
    id: 'fullname-surname-fabrication', layer: 'A/Brain·People', severity: 'P0',
    desc: 'subsetResolve fabrication guard removed — a public-figure surname sharing a family given name would silently resolve to the family member (Yitzhak Rabin → family Yitzhak)',
    file: 'src/services/people/peopleModel.ts',
    find: "return confirmed ? { status: 'resolved', id } : { status: 'conflict', id }",
    replace: "return { status: 'resolved', id }",
    owner: 'src/services/people/fullNameLookup.test.ts', expect: 'kill',
  },
  {
    id: 'mishear-suggests-garble', layer: 'A/Brain·People', severity: 'P1',
    desc: 'misheard-name similarity floor removed — genuine garble would get a confident "did you mean" suggestion instead of an honest ask-to-repeat',
    file: 'src/services/people/peopleLookup.ts',
    find: 'if (!best || best.score < minSimilarity) return null',
    replace: 'if (!best || best.score < -1) return null',
    owner: 'src/services/people/mishearSuggest.test.ts', expect: 'kill',
  },
  {
    id: 'repeat-lookup-not-guarded', layer: 'A/Brain·People', severity: 'P1',
    desc: 'session repeat-lookup guard disabled — the same failed name would be looked up + fail twice ("טורקי") instead of escalating to ask_different',
    file: 'src/services/liveTools.ts',
    find: 'if (key.length > 0) this.failedNames.add(key)',
    replace: 'if (false) this.failedNames.add(key)',
    owner: 'src/services/repeatLookupGuard.test.ts', expect: 'kill',
  },
  {
    id: 'tool-arg-oversized-unbounded', layer: 'A/Brain·Tools', severity: 'P1',
    desc: 'oversized tool-arg bound removed — a huge string reaches the date/time/name parsers (the fuzz-found pathological slow path)',
    file: 'src/services/liveTools.ts',
    find: 'const bounded = v.length > MAX_TOOL_STR ? v.slice(0, MAX_TOOL_STR) : v',
    replace: 'const bounded = v',
    owner: 'src/services/toolArgFuzz.test.ts', expect: 'kill',
  },
  {
    id: 'monitor-source-dotless-disabled', layer: 'A/Brain·Monitor', severity: 'P1',
    desc: 'output monitor no longer catches a dot-less spoken domain ("seret co il") — a spoken source would slip past',
    file: 'src/services/monitor/outputMonitor.ts',
    find: 'if (dotlessDomain.test(spoken))',
    replace: 'if (false && dotlessDomain.test(spoken))',
    owner: 'src/services/monitor/adversarialCorpus.test.ts', expect: 'kill',
  },
  {
    id: 'classified-distress-menu-disabled', layer: 'A/Brain·Monitor', severity: 'P0',
    desc: 'classified DISTRESS_MENU detector disabled — a capability menu in distress would not be flagged',
    file: 'src/services/monitor/classifiedMonitor.ts',
    find: 'if (looksLikeMenu(spoken)) return',
    replace: 'if (false && looksLikeMenu(spoken)) return',
    owner: 'src/services/monitor/classifiedCorpus.test.ts', expect: 'kill',
  },
  {
    id: 'general-search-never-answers', layer: 'A/Brain·Online', severity: 'P0',
    desc: 'general search loop never returns the judged answer — every online question would fall through to no_answer',
    file: 'src/services/online/generalSearch.ts',
    find: "if (syn.status === 'answer' && syn.answer.trim())",
    replace: "if (false && syn.status === 'answer' && syn.answer.trim())",
    owner: 'src/services/online/generalSearch.test.ts', expect: 'kill',
  },
  {
    id: 'preamble-gate-never-suppresses', layer: 'A/Brain·Voice', severity: 'P1',
    desc: 'preamble gate never suppresses — a tool-call preamble would play instead of being discarded',
    file: 'src/services/preambleGate.ts',
    find: "this.phaseVal = 'suppressed'",
    replace: "this.phaseVal = 'playing'",
    owner: 'src/services/preambleGate.test.ts', expect: 'kill',
  },
