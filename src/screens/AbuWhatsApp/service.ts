...
const TRICK_TOPICS = ['לימון','שום','מלח','קרח','חומץ','שמן זית','גרב','מגבת','ביצה','אורז מבושל','גזר','קפה','בצל','פטרוזיליה']
...
לא להגזים. לא לצחוק עליה. פשוט היא.${MANDATORY_MISTAKES}`,

  'בדיחה': `כתבי בדיחה קצרה וחמה בסגנון של מרטיטה — סבתא ארגנטינאית בת 80+ שכותבת לקבוצת הווצאפ המשפחתית.
הבדיחה חייבת: מבנה סיפורי קצר (הקדמה + פאנצ'), להיות מצחיקה ולא פתטית, להיות על חיי היומיום (רופא/שכן/חנות/ארגנטינה/משפחה/כלב/טלפון).
פורמט: 2-3 שורות. בדיחה אחת בלבד. לא להסביר אותה. לסיים ב"Ja ja ja" ו-!!!!!!
אל תחזרי על בדיחות שכתבת — כל פעם סיפור שונה לגמרי.${MANDATORY_MISTAKES}`,

  'חידה': `כתבי חידה מעניינת בסגנון מרטיטה — היא שולחת חידה לקבוצת המשפחה כאתגר.
פורמט: פתיחה "בואו נראה מי יודע!!!!" + שאלת החידה (שורה אחת) + "מי יודע?" בסוף.
החידה חייבת: להיות ברת פתרון (יש תשובה אחת ברורה), לא לילדים קטנים, לא טריוויה מוכרת מדי.
לא לכתוב את התשובה.
כל פעם חידה שונה לגמרי.${MANDATORY_MISTAKES}`,

  'טריק': `כתבי טיפ/טריק שימושי בסגנון מרטיטה — היא גילתה משהו שמשנה חיים ומשתפת בהתלהבות.
פורמט: "לא ידעתם?!!!! " + הטיפ (1-2 שורות) + תגובת התפעלות שלה.
הטיפ חייב: להיות אמיתי ושימושי (בישול/בית/בריאות/ניקיון), ספציפי עם פרטים, לא ידוע לכולם.
כל פעם טריק שונה לגמרי.${MANDATORY_MISTAKES}`,
}
...
export async function generateMessage(
  ...
  // Pick a random topic seed per call — forces the AI to pick different material every time
  const allTopics = [...JOKE_TOPICS, ...RIDDLE_TOPICS, ...TRICK_TOPICS]
  const topicSeed =
    style === 'בדיחה' ? randomFrom(JOKE_TOPICS)
    : style === 'חידה' ? randomFrom(RIDDLE_TOPICS)
    : style === 'טריק' ? randomFrom(TRICK_TOPICS)
    : randomFrom(allTopics)
  ...
  const body = {
    messages: [
      ...
      {
        role: 'user',
        content: `כתבי הודעת WhatsApp של מרטיטה על הנושא הזה: ${intent}\n\nרק את ההודעה. בלי הקדמה.\n\n[נושא השראה לסגנון: ${topicSeed}] — השתמשי בנושא הזה כנקודת מוצא, אבל כתבי בסגנון שלך.`,
      },
    ],
    temperature: 1.2,
    max_tokens: 512,
