...
import { GOLD, BRIGHT_GOLD, CREAM, getTodayStr } from './constants'
import { formatHebrewDateSlot } from './VoiceCard'

// Shared senior-first confirmation. Same calm UX for voice AND manual add:
// a clean, normalized-Hebrew summary of what will be saved (title/date/time,
// plus person if known) and three large actions. NEVER shows a raw transcript;
// only the structured fields are summarized. Editing/correction is owned by the
// host (voice fields / manual form) and triggered via onCorrect.
...
function timeLabel(time: string | null): string {
  if (!time) return 'חסר'
  return HHMM.test(time) ? time : time
}
...
export function ConfirmCard({
  draft, confirmationText, onConfirm, onCorrect, onCancel, extraTopContent,
}: {
  draft: ConfirmDraft
  ...
  return (
    <div data-testid="confirm-card" dir="rtl" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div data-testid="confirm-heading" style={{ fontSize: 18, fontWeight: 700, color: CREAM, fontFamily: "'Heebo',sans-serif" }}>
        {confirmationText ? confirmationText : 'לאישור — זה מה שאשמור:'}
      </div>
      ...
      <div data-testid="confirm-summary" style={{
        ...
        <Row label="מה" value={draft.title?.trim()} missing={!draft.title?.trim()} />
        <Row label="מתי" value={`${dateLabel ?? ''}${draft.time ? ` · ${timeLabel(draft.time)}` : ''}`.trim()} missing={!draft.date || !draft.time} />
        {draft.personName?.trim() && <Row label="עם מי" value={draft.personName.trim()} />}
        {isPast && (
