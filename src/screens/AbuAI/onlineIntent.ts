...
const ONLINE_HE_NEWS = /חדשות (היום|עכשיו|אחרונות)|מה ב?חדשות|מה קורה ב?חדשות|מה קורה בעולם|חדשות אחרונות/
const ONLINE_HE_OPEN_NOW = /מה פתוח עכשיו|פתוח עכשיו|מה פתוח (היום|בשעה)/
const ONLINE_HE_LATEST = /מה ה?חדש|מה האחרון|מה התחזית/
const ONLINE_HE_SPORTS = /כדורגל|כדורסל|מכבי|הפועל|תוצא(?:ה|ות)|מי ניצח|מי ניצחה|כמה\s+יצא|מה\s+התוצאה|משחק(?:ים|י)?\s+(?:אתמול|היום|מחר|הערב|של|ה?יום)|איזה משחקים|מונדיאל|אליפות|ליגה|גביע|נבחרת|של\s+המשחק|מה\s+היה\s+במשחק|תוצאת\s+המשחק/
