...
const CONTINUE_RE =
  /(?:^|\s)(?:תמשיכי|תמשיך|ממשיכים?|המשיכי|המשכת|לא\s+סיימת|ספרי\s+(?:לי\s+)?(?:את\s+)?ה?המשך|תחזרי\s+לזה|חזרי\s+לזה|איפה\s+(?:את\s+)?הפסקת|מאיפה\s+ש?(?:עצרת|הפסקת|היית)|הפסקת\s+ב|נעצרת|עצרת|סיימי\s+את|מה\s+ה?המשך|מה\s+היה\s+(?:אחר\s+כך|אחרי|הלאה)|עוד(?=\s*[?.!]?\s*$)|הלאה(?=\s*[?.!]?\s*$)|continue|sigue|seguí|continu[áa])/u
