...
describe('build version is bumped to B2.5', () => {
  it('APP_VERSION is the B2.5 family-intelligence marker', () => {
    const verSrc = fs.readFileSync(path.resolve(__dirname, '../../version.ts'), 'utf8')
    expect(verSrc.includes("'0.4.3-abuai-b2-5-family-intelligence'")).toBe(true)
    expect(verSrc.includes('AbuAI B2.5 — Complete Family Intelligence Matrix')).toBe(true)
  })
})
