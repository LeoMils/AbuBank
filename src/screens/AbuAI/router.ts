...
const FAMILY_PATTERNS = /מי (זה|זאת|זו|הוא|היא)\s|מי ה(בן|בת|נכד|נכדה|אמא|אבא|סבתא|סבא|אחות|אח)|מי (אמא|אבא|סבתא|סבא|אחות|אח|בת הזוג|בן הזוג|החברה|החבר) של\s|איך קוראים ל|מה הקשר (של|עם)|איך .+ קשור|הנכד שלי|הנכדה שלי|הבן שלי|הבת שלי|הילדים שלי|הנכדים שלי|ספרי לי על ה(ילדים|נכדים|בנים|בנות|נכדות|משפחה)|הילדים של |הנכדים של |כמה (ילדים|נכדים|בנים|בנות) יש|מה שלום ה(ילדים|נכדים|בנים|בנות)|מה .+ עושה|במה .+ עוסק/i
...
const CALENDAR_HEBREW_LOOSE = /מה קורה ה?שבוע|מתי (יש לי )?ה?(רופא|רופאה|דוקטור|דוקטורה|דנטיסט|תור|פגישה|אירוע)|מתי ה(תור|פגישה|אירוע).{0,12}(שלי|הבא|הבאה|הקרוב|הקרובה)|יש (אירוע|תור|פגישה).{0,12}ביומן|יש לי (פגישה|אירוע|תור) (מחר|היום|השבוע)|תזכירי לי|תזכרי לי|יש לי יום עמוס|יום עמוס|אני פנוי[הא]? השבוע|פנוי[הא]? השבוע|פנוי[הא]? מחר|מה יש .{0,8}אחרי הצהריים|יש לי .{0,8}אחרי הצהריים|זה (כבר )?ביומן|נרשם ביומן|מה קבענו|מה קבעתי (עכשיו|הרגע)|מה היה לי (שבוע שעבר|החודש|השנה)|מה היה ביומן/i
...
const CALENDAR_DATE = /מה יש (לי )?ב[־-]?\d|מה יש (לי )?ב[אבגדהוזחטיכלמנסעפצקרשת]/i
...
export function routePersonalQuery(text: string): RouteResult {
  ...
  if (CALENDAR_PAST.test(t)) {
    // Try specific date first (אתמול, date literals)
    const dateStr = parseHebrewDate(t)
    if (dateStr) return { type: 'calendar_exact_date', query: t, dateStr }
    // "שבוע שעבר" / "השנה" / "החודש" — range queries, route to upcoming
    if (/שבוע שעבר|שבוע האחרון|החודש|השנה/.test(t)) {
      return { type: 'calendar_upcoming', query: t }
    }
...
function extractFamilyName(text: string): string | null {
  const patterns = [
    ...
    /איפה (.+?) גר/i,
    /איפה גר[הא]?\s+(.+)/i,
    // Group queries: "הילדים של מור" → "מור"
    /(?:הילדים|הנכדים|הבנים|הבנות|הנכדות)\s+(?:של\s+)(.+)/i,
    /מה שלום (?:הילדים|הנכדים|הבנים|הבנות)\s+(?:של\s+)(.+)/i,
  ]
