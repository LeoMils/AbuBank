...
describe('APP_VERSION shape', () => {
  it('exposes version, buildLabel, buildDate, branchHint, commitHint', () => {
    expect(APP_VERSION.version).toBe('0.84.0-family-count-queries')
    expect(APP_VERSION.buildLabel).toBe('AbuBank — FAMILY_COUNT_QUERIES (Intelligence Parity Cycle 5, text-only via the real ExecutiveCognitiveController): closes gap F6. כמה נכדים/ילדים/נינים יש ל<X> (how many grandchildren/children/great-grandchildren) punted to the LLM — there was no count reasoner and the query carries only one family name, so routing never reached the graph. Added familyCountReasoner (grandchildrenOfPublic / greatGrandchildrenOfPublic / childrenOfPublic) + routing, so כמה נכדים יש למרטיטה → יש למרטיטה 6 נכדים: אופיר, איילון, עילי, אדר, עדי ונועם; כמה נכדים יש לי (Martita self) → לך; deterministic count + grounded list, never guessed. Evidence: familyCountQueries.test.ts 4/4 green (CODE); family regression suites 246 green; full suite green. Gap map: docs/INTELLIGENCE_GAP_MAP.md. Voice/Realtime deferred. Builds on 0.83.0.')
