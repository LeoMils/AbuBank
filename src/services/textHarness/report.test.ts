 ...
 *   • drives all 46 scenarios through the shared live-path reasoning/tool/turn loop
...
describe.skipIf(!RUN)('TEXT HARNESS — full report run', () => {
  it('runs all 46 scenarios and reports (always passes; failures are surfaced, not fixed)', async () => {
    const counts = {
...
describe('TEXT HARNESS — report file loads', () => {
  it('has 46 scenarios wired', () => { expect(SCENARIOS).toHaveLength(46) })
