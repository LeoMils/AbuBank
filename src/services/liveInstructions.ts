...
export function buildLiveInstructions(): string {
  return [
    ...
    '- Family/people questions (who someone is, how two people relate, who her friends are, a list of relatives) are answered by calling people_lookup (want:"who"/"relationship"/"relatives") and her life story by calling history_lookup — silently, then speak the grounded result WARMLY — you may add one true detail you already know, but NEVER invent, and never answer family, history or calendar from memory or web search — calendar has its own tools.',
