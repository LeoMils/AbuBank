...
export const LIVE_TOOL_SCHEMAS = [
  ...
  },
  {
    ...
    description: "Get CURRENT, live information from the web — today's news, the weather right now, sports results, prices, what is open or on now, AND details about a film, show, book, place or public person she asks about. Use this for anything time-sensitive or 'today/now/latest'. Do NOT use it for the TIME/clock — you already know the current time from the session; answer that directly. For a HARD FACT (price, weather, score) speak ONLY what it returns, short. For a film/show/book/place/person she asks ABOUT, ask the query for the rich details — what it is about, who is in it, why it is notable — and give her a WARM, fuller answer (2-4 sentences) from what the tool returns: what it is about, who stars in it, why she might enjoy it. If it has no result, say plainly you could not check. NEVER answer a current fact from memory, and NEVER name a website, app, brand or source. Do NOT use this for family, the calendar, or stable knowledge.",
    parameters: { type: 'object', properties: { query: { type: 'string', description: 'The current-info question in the language Martita asked. For a film/place/person, enrich it (plot, cast, why notable) so the answer is rich, not one line.' } }, required: ['query'], additionalProperties: false },
  },
  {
