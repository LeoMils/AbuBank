...
|---|---|---|---|---|---|
| 0.151.0 | n/a (intake) | +10 | — | **INTAKE REBUILD s12 · fail-closed understanding (obligation #9).** `interpretUtterance` bounded timeout (hanging provider → unknown); malformed/partial/provider-down/unsupported-op → safe unknown. `decideIntakeAction` policy: act / clarify(one question) / decline — never acts on empty/contradictory/unsure structured meaning. | CODE — understandingFailClosed 10/10 (incl. fake-timer timeout) + FULL suite **11562 pass / 0 regressions**, tsc + build. NOT device-proven. |
| 0.150.0 | n/a (intake) | +5 | — | **INTAKE REBUILD s11 · shadow-validated retirement (obligations #2/#6/#13/#14).** Reusable shadow harness (`intakeShadow.ts`) ran legacy pattern intake vs the seam over 1419 turns: **agree=1101, recovered=318, regressed=0, disagree=0** → seam is a proven strict superset. Retired the legacy REL from the live path (`answerFamilyRelation` seam-only; legacy quarantined in `legacyFamilyIntake.ts` for shadow only) — one intake per capability. Added the `ממי X גרושה` shape to the seam. Obligations recorded as repo law. | CODE — intakeShadow 5/5 (0 regress/0 disagree/318 recovered) + FULL suite **11552 pass / 0 regressions**, tsc + build. NOT device-proven. |
| 0.149.0 | n/a (intake) | +4 | — | **INTAKE REBUILD s10 · verification regime (P1–P8 complete, CODE side).** Full corpus clean (387 files / 11546 pass / 0 regress). Internal free-language SIMULATION: 300+ generated utterances (morphology × garble × members × paths) asserting invariants (never wrong person, never crash, resolved-name titles, no denial). 10-line Hebrew Leo test card. | CODE + local sim. **HONEST LIMIT**: 200-session run through the DEPLOYED app + on-device voice = PREVIEW/PHYSICAL, NOT done (deploy human-gated; preview URL unfetchable here). NOT device-proven. |
| 0.148.0 | n/a (intake) | +5 | — | **INTAKE REBUILD s9 · P7 + P8.** P7: a factual correction of a prior ONLINE answer ("לא נכון"/"טעית") re-searches that topic instead of blind-agreeing (focus=online → re-run online tool; deterministic domains keep priority; plain "לא" excluded). P8: killed the "אבחון הקול הועתק" chat-bubble spam (dedup). | CODE — correctionVerification 5/5 (incl. live re-search) + FULL suite **11542 pass / 0 regressions**, tsc + build. P8 = BROWSER (by inspection). Online quality = PREVIEW. NOT device-proven. |
| 0.147.0 | n/a (intake) | +6 | — | **INTAKE REBUILD s8 · P6 no-fabrication.** Pre-emission guard in `runtimeFullTurn`: an LLM/fallback answer asserting a specific appointment (frame + date/clock, the "1 באוקטובר" class) is neutralized to an honest calendar deferral. Precise — historical dates + prose pass; deterministic/online trusted. | CODE — guard 6/6 (incl. live LLM-invents-appointment scrubbed) + FULL suite **11536 pass / 0 regressions**, tsc + build. NOT device-proven. |
| 0.146.0 | n/a (intake) | +13 | — | **INTAKE REBUILD s7 · P5 ledger width.** Explicit-remember covers EVERY chapter kind via `extractExplicitFact` (education/hobby/event + generic STORY catch-all), gated to KNOWN family persons (Martita's own "שלי"/first-person stays personal-memory). Privacy holds (medical/financial declined). WIDTH is explicit-only — shared soft-confirm `extractChange` unchanged → no eval drift (a regression via the shared path was caught + reverted). | CODE — ledgerWidth 13/13 + FULL suite **11529 pass / 0 regressions**, tsc + build. NOT device-proven. |
| 0.145.0 | n/a (intake) | +4 | — | **INTAKE REBUILD s6 · P4 calendar audit.** Full-journey regression (create→query→edit→delete→recreate round-trips the store; multiple meetings/person allowed; title carries resolved name) + STANDING capability-denial probe (a "לא יכולה/can't" on read/search/edit/delete = hard failure). Test-only. | CODE — journey 4/4 + FULL suite **11516 pass / 0 regressions**, tsc + build. NOT device-proven. |
